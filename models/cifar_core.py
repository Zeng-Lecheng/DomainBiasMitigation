...
class CifarModel():
    ...
    def _final_test(self, loader):
        ...
        return test_result, class_count
    ...
    def test(self):
        ...
        test_color_result, color_class_count = self._final_test(self.test_color_loader)
        test_gray_result, gray_class_count = self._final_test(self.test_gray_loader)
        bias_sum = 0

        # Calculate bias
        for i in range(10):
            color_class_count[i] = color_class_count[i] + color_class_count[
                i + 10]  # combine the smae class of different domains
            gray_class_count[i] = gray_class_count[i] + gray_class_count[i + 10]
            color_class = color_class_count[i]
            gray_class = gray_class_count[i]
            bias_sum += max(color_class, gray_class) / (color_class + gray_class)
        bias = bias_sum / 10 - 0.5
        ...
        info = ('Bias: {}\n'
                'Test on color images accuracy: {}\n'
                'Test on gray images accuracy: {}\n'.format(bias, test_color_result['accuracy'],
